...
function targetExists(storyboard, targetKind, targetId) {
  return targetKind === "frame" && FRAME_ID_RE.test(targetId) && Object.hasOwn(storyboard.frames, targetId);
}
...
export function replaceStoryboardTargetReferences(storyboard, project, { targetKind, targetId, references }) {
  ...
  return { storyboard: next, references: next.frames[targetId].references };
}
...
export function storyboardSummary(storyboard) {
  return {
    ...
    effectiveReferences: Object.values(storyboard.frames || {}).reduce((total, frame) => total + (frame?.references?.length || 0), 0),
