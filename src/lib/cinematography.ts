...
function planFromShot(shot: Shot, now: number): ShotCinematographyPlan {
  return { id: `cine-plan:${shot.id}`, shotId: shot.id, sceneId: shot.sceneId, lens: shot.lens || "35mm", framing: shot.camera || shot.type, cameraHeight: shot.type === "insert" ? "table height" : "eye line", movement: shot.cameraMove || "motivated static", focus: shot.type === "closeup" ? "shallow emotional focus" : "geography readable", lighting: "motivated by approved research manifesto", texture: "35mm grain / production texture", geography: "preserve scene screen direction", rhythm: `${shot.durationSec}s ${shot.type}`, motif: shot.emotion || "continuity", status: "DRAFT", approvedVersionId: null, sourceFingerprints: [fp(shot.id, shot, null)] };
}
...
export function runCinematographyQa(state: CinematographyState, visual?: VisualDevelopmentState, now = Date.now()): CinematographyQaReport {
  ...
  for (const [sceneId, plans] of byScene) {
    ...
    if (closeups.length > Math.max(2, plans.length - 1)) findings.push({ id: `cineqa:${sceneId}:closeups`, severity: "warning", category: "REPETITION", scopeId: sceneId, message: "Coverage leans on repeated close/portrait framing.", recommendation: "Add geography, insert, or motivated negative-space coverage." });
