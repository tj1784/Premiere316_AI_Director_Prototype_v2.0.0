...
export function createProjectStorage(options: ProjectStorageOptions = {}): StateStorage<Promise<void>> {
  ...
  const report = (operation: ProjectStorageError["operation"], key: string, cause: unknown) => {
    ...
    try { options.onError?.(detail); } catch { /* The storage result is independent of notification handlers. */ }
    try { browser?.dispatchEvent(new CustomEvent(PROJECT_STORAGE_ERROR_EVENT, { detail })); } catch { /* SSR/test or unavailable events. */ }
  };

  let legacy = options.legacyStorage;
  if (legacy === undefined) {
    try { legacy = browser?.localStorage ?? null; } catch { legacy = null; }
  }
  let backend = options.backend;
  if (backend === undefined) {
    try { backend = browser?.indexedDB ? indexedDbBackend(browser.indexedDB) : null; } catch { backend = null; }
  }
  if (browser && options.backend === undefined && backend && ['localhost', '127.0.0.1', '[::1]'].includes(browser.location.hostname)) {
  ...
  const removeLegacyAfterCommit = (key: string) => {
    ...
    try { legacy?.removeItem(key); } catch { /* Durable save succeeded; retain the harmless legacy backup if cleanup is denied. */ }
  };

  return {
    getItem: (key) => ordered(async () => {
      try {
        if (backend) {
    ...
    setItem: (key, value) => ordered(async () => {
      try {
        if (unreadableKeys.has(key)) throw new Error("Saving is paused until existing pictures can be read.");
        if (backend) {
    ...
    removeItem: (key) => ordered(async () => {
      try {
        if (unreadableKeys.has(key)) throw new Error("Removing saved pictures is paused until existing pictures can be read.");
...
function indexedDbBackend(factory: IDBFactory): ProjectStorageBackend {
  ...
  const transaction = async <T>(mode: IDBTransactionMode, operation: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> => {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      tx.onabort = () => reject(tx.error ?? request.error ?? new Error("Picture storage transaction was aborted."));
      tx.onerror = () => reject(tx.error ?? request.error ?? new Error("Picture storage transaction failed."));
    });
  };
  ...
  return {
    getItem: async (key) => {
      ...
      if (typeof value !== "string") throw new Error("Saved picture storage has an unexpected format.");
      return value;
    },
