...
import { assetPromptContext, compactAssetPromptContext, assertAssetDevelopmentReady, assertCharacterSheetPrompt, isVisualAsset, type AssetPromptSource } from "./asset-prompt-context.ts";
...
export async function writeGroundedAssetPrompts(picture: Picture, runtime: MoviePlanRuntime, countTokens: (prompt: string) => Promise<number>, replaceEditedPrompts = false, onPicture?: (picture: Picture) => void, onStatus?: (message: string) => void, onlyAssetId?: string, forceRewrite = false): Promise<Picture> {
  assertAssetDevelopmentReady(picture);
  ...
  for (const [index, asset] of assets.entries()) {
    const shared = compactAssetPromptContext(contexts[index].context);
    ...
    prompt: JSON.stringify({
      ...
      relatedAssets: shared.relatedAssets,
      relatedAssetColumns: shared.relatedAssetColumns,
      canonicalParents: shared.canonicalParents,
      referencePackageInstructions: "Respect the target referenceRequirements, importedSpecification.reference and continuity states. This request writes one reviewable image prompt; it does not complete every view in a reference package or approve an asset. Attached reference descriptions are research or design context, never a substitute for approved canonical parent media.",
      assets: [{ assetId: asset.id, imageContract: shared.asset.imageContract, linkedSceneIds: shared.scenes.map((scene) => scene.id), cinematography: shared.cinematography.shots, ...(!replaceEditedPrompts ? { currentUserPrompt: picture.assetImagePrompts?.[asset.id] ?? null } : {}) }],
      output: "Write ONE " + (imageEngineId === "krea-2" ? "KREA 2 RAW natural-language text-to-image" : "FLUX.2-dev-friendly reference image") + " prompt, AT LEAST 1024 TOKENS (target 1600–2400 tokens, maximum 3500). Return assetId, promptParagraphs, and a short sourceQuote copied verbatim from a linked scene. promptParagraphs must contain TEN fully written natural-language visual paragraphs, with a combined target of 1600–2400 tokens. Let every paragraph end at a complete sentence; vary paragraph lengths naturally instead of forcing equal character counts. The app joins these model-written paragraphs verbatim as the image prompt. Cover ten distinct visible aspects: subject identity and period; silhouette and proportions; anatomy or physical construction; materials and surface texture; wardrobe or component design; palette and wear; scene placement and scale; motivated lighting and readable exposure; lens, viewpoint and framing; continuity with related assets and the film's existing visual style. For a prop, adapt these aspects to the prop rather than inventing a person. Attached reference annotations may supply design context; never infer that this text writer inspected unseen pixels. The screenplay, visual development and cinematography determine the rendered medium and lighting. Use positive concrete descriptions, not weighted tags, sampler settings or a separate negative prompt. Every paragraph must add useful visible detail; no padding, repetition, invented story events, modern substitutions or unrelated objects. Explicitly carry all relevant screenplay, cinematography and shared identity constraints into the prompt. Do not put explanatory source citations or text overlays inside the rendered image.",
      ...
      finalTargetReminder: `This request is for ${asset.name} (${asset.id}) ONLY. Follow the complete targetAsset specification above. Secondary relatedAssets are concise continuity summaries; do not design or replace those subjects in this request.`,
