...
import { stableHash } from "./dependency-graph.ts";
...
export function inspectAudioTakeQC(input: {
  ...
  const checks = [
    {
      id: "exists",
      ok: Boolean(input.probe?.byteLength && input.probe.byteLength > 0),
      message: "Audio file must exist and be nonzero.",
    },
    {
      id: "probe",
      ok: Boolean(input.probe?.ok),
      message: input.probe?.ok
        ? "Audio probe succeeded."
        : "No valid audio probe; generation did not produce audio.",
    },
    {
      id: "duration",
      ok: Boolean(input.probe?.durationSec && input.probe.durationSec > 0),
      message: "Duration must be greater than zero.",
    },
    {
      id: "hash",
      ok: Boolean(input.mediaSha256 && /^[a-f0-9]{64}$/.test(input.mediaSha256)),
      message: "SHA-256 of durable audio is required.",
    },
    {
      id: "origin",
      ok: input.origin === "imported" || Boolean(input.probe?.ok),
      message: "Fail-closed generated audio cannot satisfy canonical.",
    },
  ];
...
export function segmentDialogue(picture: Picture, now = Date.now()): DialogueLine[] {
  const fromVoices: DialogueLine[] = picture.voices.map((voice: VoiceTake, index) => ({
    ...
    characterId:
      picture.characters.find((character) => character.name === voice.character)?.id ?? null,
  ...
  // Character arcs and emotional beats are not authorized spoken dialogue.
  return [];
}
...
export function hydratePictureAudio(picture: Picture): AudioWorkspace {
  const current =
    picture.audio && picture.audio.schemaVersion === 1 ? picture.audio : emptyAudioWorkspace();
  return {
    schemaVersion: 1,
...
export function enqueueAudioJob(
...
export function failClosedAudioJob(
  workspace: AudioWorkspace,
  jobId: string,
  reason: string,
  now = Date.now(),
  ...
  for (const line of workspace.lines) {
    if (
      workspace.takes.some(
        (take) =>
          take.lineId === line.id && (take.status === "CANONICAL" || take.origin === "imported"),
      )
    )
      continue;
    workspace = enqueueAudioJob(workspace, {
      pictureId: picture.id,
      kind: "dialogue",
      engineId,
      lineId: line.id,
      now: now + workspace.jobs.length,
    });
    workspace = failClosedAudioJob(
      workspace,
      workspace.jobs[workspace.jobs.length - 1].id,
      reason,
      now + workspace.jobs.length,
    );
  }
  return workspace;
}
...
export function queueMissingScore(picture: Picture, now = Date.now()): AudioWorkspace {
  ...
  for (const cue of workspace.cues) {
    if (
      workspace.takes.some(
        (take) =>
          take.cueId === cue.id && (take.status === "CANONICAL" || take.origin === "imported"),
      )
    )
      continue;
    workspace = enqueueAudioJob(workspace, {
...
export function recordImportedAudioTake(
  workspace: AudioWorkspace,
  input: {
    pictureId: string;
    kind: AudioKind;
  ...
  if (!/^[a-f0-9]{64}$/.test(input.mediaSha256))
    throw new Error("Imported audio requires a SHA-256.");
  ...
  const take: AudioTake = {
    id: `audiotake:import:${crypto.randomUUID()}`,
    ...(input.cueId
      ? { cueFingerprint: stableHash(workspace.cues.find((c) => c.id === input.cueId)) }
      : {}),
    jobId: null,
    pictureId: input.pictureId,
    lineId: input.lineId ?? null,
...
export function reviewAudioTake(
  workspace: AudioWorkspace,
  takeId: string,
  decision: "reject" | "canonical",
  reason: string,
  now = Date.now(),
): AudioWorkspace {
  ...
  if (decision === "canonical") {
    if (
      take.cueFingerprint &&
      take.cueFingerprint !== stableHash(workspace.cues.find((c) => c.id === take.cueId))
    )
      throw new Error(
        "Cue changed since this take was imported. Review an explicitly rebound or new take against the current cue.",
      );
    if (take.origin !== "imported" && !(take.origin === "native-generated" && take.generationProvenance && ["ace-step-1.5-xl-sft", "stable-audio-3-small-sfx"].includes(take.engineId)))
      throw new Error(
        "Canonical audio approval requires imported durable media until a native TTS/Music runtime exists.",
      );
    if (!take.mediaSha256 || !take.probe?.ok)
      throw new Error("Canonical audio approval requires probed durable media.");
    return {
      ...workspace,
      takes: workspace.takes.map((item) =>
        bindKey(item) === bindKey(take)
          ? item.id === takeId
            ? {
                ...item,
                status: "CANONICAL",
            ...
            : item.canonical
              ? {
                  ...item,
                  canonical: false,
                  status: item.status === "CANONICAL" ? "NEEDS_REVIEW" : item.status,
                  updatedAt: now,
                }
              : item
          : item,
  ...
  return {
    ...
    takes: workspace.takes.map((item) =>
      item.id === takeId
        ? { ...item, status: "REJECTED", canonical: false, reviewReason: reason, updatedAt: now }
        : item,
    ),
  };
}
...
export function restoreAudioWorkspace(
  workspace: AudioWorkspace | null | undefined,
): AudioWorkspace {
  const hydrated = workspace && workspace.schemaVersion === 1 ? workspace : emptyAudioWorkspace();
  return {
    ...hydrated,
    jobs: hydrated.jobs.map((job) =>
      job.status === "running"
        ? {
            ...job,
            status: "queued" as const,
            error:
              "Recovered after restart; running audio job was re-queued instead of duplicated.",
