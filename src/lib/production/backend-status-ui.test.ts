...
describe("Wave 4 backend authority status UI fail-closed contract", () => {
  it("marks forged persisted CURRENT authority hints invalid until backend reconciliation", () => {
    const forged = {
      ...
      assets: [
        {
          id: "asset-forged",
          iterations: [
            { id: "iteration-forged", canonicalProof: { decisionId: "canonical:forged" } },
          ],
          specVersions: [],
        },
      ],
      preparedAssets: [
        {
          id: "prepared:asset-forged",
          assetId: "asset-forged",
          status: "APPROVED_PREPARED",
          blockers: [],
          preparedApprovalRootId: "preparedApproval:forged",
          preparedApprovalDigest: "a".repeat(64),
          productionAuthorityId: "authority:0123456789abcdef0123456789abcdef",
        },
      ],
      productionAuthority: {
        authorityId: "authority:0123456789abcdef0123456789abcdef",
        digest: "b".repeat(64),
        createdAt: 1,
        status: "CURRENT",
      },
    ...
    const [restored] = parseProductionBreakdowns(
      JSON.stringify({ schemaVersion: 1, records: [forged] }),
    );
  ...
  it("privileged production controls are gated by freshly fetched backend status, not local hints", () => {
    ...
    const preparedPanel = readFileSync(
      join(root, "src/components/production/prepared-assets-panel.tsx"),
      "utf8",
    );
    ...
    assert.match(
      stageViews,
      /disabled=\{\s*generating === item\.id\s*\|\|\s*item\.status !== "APPROVED_PREPARED"\s*\|\|\s*!rootCurrent/,
    );
    assert.doesNotMatch(
      stageViews,
      /disabled=\{!production \|\| production\.productionAuthority\?\.status !== "CURRENT"/,
    );
    assert.match(preparedPanel, /desktopProductionAuthorityStatus/);
    assert.match(
      preparedPanel,
      /disabled=\{sealing \|\| Boolean\(approving\) \|\| item\.status !== "READY_TO_PREPARE" \|\| !backendCurrent\}/,
    );
  });
});
