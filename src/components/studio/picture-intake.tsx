    ...
    <div className="min-h-dvh bg-bg">
      ...
      <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 sm:py-10">
        ...
        <form id="picture-intake" className="mt-8 grid gap-8" onSubmit={async (event) => { event.preventDefault(); setSubmitted(true); if (!validation.valid || directionBusy) return; setSaving(true); try { await onCreate(intake); } finally { setSaving(false); } }}>
          ...
          <section className="rounded-lg bg-elevated p-5 shadow-[var(--shadow-border)]">
            <p className="text-[11px] tracking-[0.2em] text-subtle uppercase">Film contract · Optional</p>
            <p className="mt-2 text-xs leading-relaxed text-muted">Record only choices you have made. These directions appear in the Movie Script Bible and writer brief.</p>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <TextField label="Moral question" value={intake.moralQuestion ?? ""} onChange={(value) => patch("moralQuestion", value)} rows={3} />
              <FormField label="Language" value={intake.language ?? ""} onChange={(value) => patch("language", value)} />
              <FormField label="Requested delivery format" value={intake.deliveryFormat ?? ""} onChange={(value) => patch("deliveryFormat", value)} />
              <FormField label="Requested video codec" value={intake.deliveryCodec ?? ""} onChange={(value) => patch("deliveryCodec", value)} />
              <TextField label="Continuity policy" value={intake.continuityPolicy ?? ""} onChange={(value) => patch("continuityPolicy", value)} rows={3} />
              <TextField label="Voice policy" value={intake.voicePolicy ?? ""} onChange={(value) => patch("voicePolicy", value)} rows={3} />
              <TextField label="Score strategy" value={intake.scoreStrategy ?? ""} onChange={(value) => patch("scoreStrategy", value)} rows={3} />
            </div>
