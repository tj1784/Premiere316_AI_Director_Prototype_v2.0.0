...
export function VisualDirectionField({ value, onChange, disabled = false, onBusy, writerId }: { value?: VisualDirection; onChange: (value: VisualDirection | undefined) => void; disabled?: boolean; onBusy?: (busy: boolean) => void; writerId?: string }) {
  ...
  useEffect(() => { let live = true; setPreview(""); setBoardExpanded(false); if (value?.boardId) void boardImage(value.boardId).then(src => { if(live) setPreview(src); }).catch(e => { if(live) setError(String(e)); }); return () => { live = false; }; }, [value?.boardId]);
  ...
  return <section aria-label="Visual direction" className="grid gap-3 rounded-lg bg-elevated p-4 shadow-[var(--shadow-border)]">
    ...
    {preview ? (
      <div className="min-w-0 rounded-md border border-border bg-bg/40 p-2">
        <button
          type="button"
          className="w-full text-left"
          aria-label={boardExpanded ? "Fit visual direction board" : "Enlarge visual direction board"}
          aria-expanded={boardExpanded}
          onClick={() => setBoardExpanded((expanded) => !expanded)}
        >
          <img
            src={preview}
            alt="Combined visual direction reference board"
            className={`w-full rounded-md object-contain transition-[max-height] duration-300 motion-reduce:transition-none ${boardExpanded ? "max-h-[80vh]" : "max-h-96"}`}
          />
          <span className="mt-2 block text-right text-xs text-muted">
            {boardExpanded ? "Fit board in workspace" : "Enlarge board in workspace"}
          </span>
        </button>
      </div>
    ) : null}
