    ...
    {ASSET_REVIEW_GROUPS.filter((group) => category === group.id || category === "all" && allAssets.some((asset) => assetReviewGroup(asset.category).id === group.id)).map((group) => <section key={group.id} aria-label={`${group.label} assets`} className="grid gap-4">
      ...
      {assetReviews.filter(({ asset }) => assetReviewGroup(asset.category).id === group.id).map(({ asset, source, prompt, sourceCurrent, current, latest }) => {
        return <article key={asset.id} className="grid w-full min-w-0 flex-[1_1_16rem] gap-3 rounded-lg bg-elevated p-4 shadow-[var(--shadow-border)] sm:max-w-[22rem]">
          ...
          {asset.references.length ? <div className="flex flex-wrap gap-2">{asset.references.map(ref => <button type="button" key={ref.id} aria-label={`Open reference ${ref.name}`} onClick={() => setPreview({ name: ref.name, uri: ref.previewUri ?? ref.uri })}><img src={ref.previewUri ?? ref.uri} alt={ref.name} className="size-20 rounded-md object-contain" /></button>)}</div> : null}
          ...
          {latest?.mediaUri ? <button className="grid gap-2 text-left" aria-label={`Enlarge ${asset.name} image`} onClick={() => setPreview({ name: asset.name, uri: latest.previewUri ?? latest.mediaUri! })}><img src={latest.previewUri ?? latest.mediaUri} alt={asset.name} className="aspect-square w-full rounded-md object-contain" /><span className="text-sm text-accent">Open image · {latest.width} × {latest.height}</span></button> : <p className="py-8 text-sm text-muted">Image generation pending.</p>}
          ...
          {asset.references.length ? <details className="text-sm"><summary className="cursor-pointer">Attached visual references ({asset.references.length})</summary><div className="mt-2 grid grid-cols-2 gap-2">{asset.references.map((reference) => <figure key={reference.id}><img src={reference.previewUri ?? reference.uri} alt={reference.name} className="aspect-square w-full object-contain" /><figcaption className="mt-1 text-xs text-muted">{reference.name}</figcaption></figure>)}</div></details> : null}
