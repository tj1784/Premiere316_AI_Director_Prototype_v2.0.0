...
export function ProjectFiles({ picture }: { picture: Picture }) {
  ...
  return (
    <section className="project-files-panel" aria-label={`${picture.title} project files`}>
      <p className="text-sm text-muted">Files saved with this picture, grouped by category.</p>
      {error ? (
        ...
        <>
          ...
          <div className="project-files-list">
            ...
            {entries.length > limit ? (
              <Button variant="ghost" className="mt-4 w-full" onClick={() => setLimit(limit + 48)}>
                Show more files
              </Button>
            ) : null}
          </div>
