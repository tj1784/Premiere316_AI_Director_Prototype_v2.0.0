...
export function CharacterMediaPanel({
  ...
  const isOpen = (kind: NonNullable<typeof pane>["kind"]) =>
    pane?.recordId === record.id && pane.kind === kind;
  const open = (kind: NonNullable<typeof pane>["kind"]) => setPane({ recordId: record.id, kind });
  const openInventory = () => {
  ...
  return (
    ...
    >
      {pane ? (
        <section
          className="character-media-inline-detail"
          aria-label={`${record.name} ${pane.kind} panel`}
        >
          <header className="character-media-detail-heading">
            <div>
              <h3 tabIndex={-1} ref={paneHeadingRef}>
                {isOpen("preview")
                  ? "Reference preview"
                  : isOpen("import")
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              aria-label="Return to visual and voice references"
              onClick={() => setPane(null)}
            >
              <X size={18} />
            </Button>
          </header>
          <div className="character-media-detail-content">
            {isOpen("preview") && (
              <>
                {selected && (
                  <div className="character-media-full-preview">
                    ...
                    <div className="character-media-actions">
                      <Button
                        variant="secondary"
                        onClick={
                          selected.origin === "board" ? openVisualDevelopment : openInventory
                        }
                      >
              ...
              <>
                {asset && picture.production && (
                  <div className="character-media-import">
                    <p>
                      Choose an image iteration or a design reference. Review the result before
                      using it as an approved identity.
                    </p>
              ...
              <>
                {asset && (
                  <>
                    {voice?.approved && (
                      <p className="character-media-note">
                        Approved reference: {voice.approved.id} · revision{" "}
                        {voice.approved.revision ?? 0}
                        {voice.approved.design?.language
                          ? ` · ${voice.approved.design.language}`
                          : ""}
                      </p>
                    )}
        ...
        <>
          ...
          >
            <div className="character-media-heading">
              <h3 id={`character-visual-${record.id}`} tabIndex={-1} ref={mediaHeadingRef}>
            ...
            {selected ? (
              <>
                ...
                <div className="character-media-caption" aria-live="polite">
                  ...
                  <span
                    className={
                      selected.approved ? "character-media-approved" : "character-media-note"
                    }
                  >
            ...
            {asset && (
              <p className="character-media-note">
                {asset.canonicalApproved
                  ...
                  : "Specification awaiting approval"}
            ...
            {!asset && (
              <p className="character-media-note">
                This character has no linked production asset. Production image approvals are not
                yet available.
              </p>
            )}
          ...
          >
            ...
            <div className="character-media-voice-status">
              <span
                className={voice?.approved ? "character-media-approved" : "character-media-note"}
              >
                {voice?.approved ? (
                  <>
                    <Check size={13} />
                ...
                {audioUri ? (
                  ...
                  <>
                    <p role="alert" className="character-media-error">
                      Preview and original recording are unavailable. Open voice iterations to
                      inspect or replace this reference.
                    </p>
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() =>
                        setFailedVoice((current) => ({ ...current, [audition.id]: [] }))
                      }
                    >
                      Retry recording
                    </Button>
