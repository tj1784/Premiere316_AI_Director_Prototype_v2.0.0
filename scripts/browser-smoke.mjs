...
try {
  const browserCandidates = [
    process.env.BROWSER_SMOKE_EXECUTABLE,
  ...
  for (const vp of VIEWPORTS) {
    ...
    page.on("console", (msg) => {
      if (msg.type() === "error") {
        ...
        // The platform injects this bridge and the restricted smoke browser
        // cannot reach grok.com. Keep the bridge in production; ignore only
        // this precise network-policy failure in local verification.
        if (source === "https://grok.com/grok-app-builder/extensions.js" && msg.text().includes("ERR_NETWORK_ACCESS_DENIED")) return;
        errors.consoleErrors.push(source ? `${msg.text()} (${source})` : msg.text());
      }
    });
    ...
    const readySelector = process.env.BROWSER_SMOKE_READY_SELECTOR;
