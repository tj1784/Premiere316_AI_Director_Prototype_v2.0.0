...
function main(argv) {
  ...
  // Windows does not search npm's `.cmd` shims when `spawn` is called without
  // a shell. Run Vite's JavaScript entry directly so the same wrapper works on
  // Windows and Unix without enabling shell parsing.
  const executable = command === "vite" ? process.execPath : command;
  const childArgs = command === "vite" ? [join(projectRoot(), "node_modules", "vite", "bin", "vite.js"), ...args] : args;
