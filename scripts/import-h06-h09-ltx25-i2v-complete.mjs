...
export const PACKAGE_ID = "h06_h09_ltx25_i2v_complete_v1";
export const FPS = 24;
...
export const PACKAGE_DEFINITIONS = Object.freeze({
  H06: Object.freeze({
    chapterId: "H06",
    archiveName: "H06_LTX25_I2V_COMPLETE.zip",
    archiveSha256: "40dbdcb5d49d118bdaa10bcceb94eed15af6a332e0ca6241333ffcb50e2e6c5a",
    archiveBytes: 90741200,
    rootEntry: "H06_LTX25_I2V_COMPLETE",
    csv: "H06_FIRST_FRAME_BINDINGS.csv",
    prompts: "H06_I2V_PROMPTS.md",
    sources: "H06_SOURCE_REFERENCES.md",
    frameShaJson: "SHA256_MANIFEST.json",
    videoPlans: "H06_VIDEO_PLANS_I2V.json",
    expectedClips: 12,
    expectedFrames: 36,
    expectedReferenceBindings: 213,
    filenameSuffix: "h06-i2v-complete",
    referenceSchema: "single"
  }),
  H07: Object.freeze({
    chapterId: "H07",
    archiveName: "H07_LTX25_I2V_COMPLETE.zip",
    archiveSha256: "1c0ef349ec435b76dee32c4ba70a0e0825bedc3ee19dadde3daf3d49f1e456ed",
    archiveBytes: 108894489,
    rootEntry: "work/chapters/H07_LTX25_I2V_COMPLETE",
    csv: "H07_FIRST_FRAME_BINDINGS.csv",
    prompts: "H07_I2V_PROMPTS.md",
    sources: "H07_SOURCE_REFERENCES.md",
    shaManifest: "MANIFEST_SHA256.txt",
    frameShaJson: "SHA256_MANIFEST.json",
    frameShaCsv: "H07_FRAME_SHA256.csv",
    videoPlans: "H07_VIDEO_PLANS_I2V.json",
    expectedClips: 15,
    expectedFrames: 45,
    expectedReferenceBindings: 267,
    expectedReferenceRelations: Object.freeze({
      storyboard_declared_and_loaded_for_generated_frame: 195,
      storyboard_declared: 63,
      loaded_for_generated_frame: 9
    }),
    filenameSuffix: "h07-i2v-complete",
    referenceSchema: "storyboard_generation_union"
  }),
  H08: Object.freeze({
    chapterId: "H08",
    archiveName: "H08_LTX25_I2V_COMPLETE.zip",
    archiveSha256: "f4e21661a0bafcf01720363681d9415827204cfcf6c49373c854ce8946005e3b",
    archiveBytes: 78524858,
    rootEntry: "H08_LTX25_I2V_COMPLETE",
    csv: "H08_FIRST_FRAME_BINDINGS.csv",
    prompts: "H08_I2V_PROMPTS.md",
    sources: "H08_SOURCE_REFERENCES.md",
    frameShaJson: "SHA256_MANIFEST.json",
    videoPlans: "H08_VIDEO_PLANS_I2V.json",
    expectedClips: 12,
    expectedFrames: 36,
    expectedReferenceBindings: 189,
    filenameSuffix: "h08-i2v-complete",
    referenceSchema: "single"
  }),
  H09: Object.freeze({
    chapterId: "H09",
    archiveName: "H09_LTX25_I2V_COMPLETE.zip",
    archiveSha256: "00dcc939f70612154f58aebd5e9e0683a675b056f09750ca3ee719c56b65ef99",
    archiveBytes: 38950695,
    rootEntry: "H09_LTX25_I2V_COMPLETE",
    csv: "H09_FIRST_FRAME_BINDINGS.csv",
    prompts: "H09_I2V_PROMPTS.md",
    sources: "H09_SOURCE_REFERENCES.md",
    frameShaJson: "SHA256_MANIFEST.json",
    videoPlans: "H09_VIDEO_PLANS_I2V.json",
    continuityLocks: "H09_CONTINUITY_LOCKS.md",
    expectedClips: 6,
    expectedFrames: 18,
    expectedReferenceBindings: 99,
    filenameSuffix: "h09-i2v-complete",
    referenceSchema: "single"
  })
});
...
function parseSourceReferences(markdown, chapterId, referenceSchema) {
  ...
  for (let index = 0; index < headings.length; index += 1) {
    ...
    for (const line of body.split("\n")) {
      ...
      if (!match) continue;
      if (!section) throw new Error(`${chapterId} source-reference bullet appears outside a declared H07 relation section: ${clipId}`);
...
function loadPackage(definition, archivePath) {
  ...
  try {
    ...
    const sourceSections = parseSourceReferences(fs.readFileSync(sourcesFile, "utf8"), definition.chapterId, definition.referenceSchema);
    ...
    for (const row of rows) row.referenceDeclarations = referenceDeclarationsForRow(definition, row, sourceSections);
    const referenceBindingCount = rows.reduce((sum, row) => sum + row.referenceDeclarations.length, 0);
    if (referenceBindingCount !== definition.expectedReferenceBindings) {
      throw new Error(`${definition.chapterId} expected ${definition.expectedReferenceBindings} per-frame provenance bindings; received ${referenceBindingCount}`);
    }
    if (definition.expectedReferenceRelations) {
      const actualRelations = {};
      for (const row of rows) for (const reference of row.referenceDeclarations) actualRelations[reference.sourceRelation] = (actualRelations[reference.sourceRelation] || 0) + 1;
      if (JSON.stringify(actualRelations) !== JSON.stringify(definition.expectedReferenceRelations)) {
        throw new Error(`${definition.chapterId} storyboard/generation provenance relation counts differ from the audited package contract`);
      }
    }
    ...
    if (definition.sourceExtract) {
      ...
      if (sourceExtract.chapter !== definition.chapterId || sourceExtract.clips !== clipIds.length || sourceExtract.segments?.length !== rows.length) throw new Error(`${definition.chapterId} source extract count mismatch`);
      ...
      for (const row of rows) {
        ...
        if (!item || Number(item.start_frame) !== timing.start || Number(item.length_frames) !== timing.length || `first_frames/${item.filename}` !== row.first_frame) {
          throw new Error(`${definition.chapterId} source extract differs from CSV: ${key}`);
        }
        const declared = sourceSections.get(row.clip_id).map((reference) => reference.filename);
        if (JSON.stringify(item.refs || []) !== JSON.stringify(declared)) throw new Error(`${definition.chapterId} source extract references differ for ${key}`);
      }
    }

    let packagePlans = null;
    ...
    if (definition.videoPlans) {
      packagePlans = JSON.parse(fs.readFileSync(safeSourcePath(extracted.packageRoot, definition.videoPlans), "utf8"));
      const plans = Object.values(packagePlans.videoPlans || {});
      if (packagePlans.chapter?.id !== definition.chapterId || Number(packagePlans.master?.fps) !== FPS || packagePlans.master?.normalizedFrameSize !== "1920x804") {
        throw new Error(`${definition.chapterId} package plan chapter/FPS/frame-size contract is invalid`);
      }
      if (plans.length !== clipIds.length || new Set(plans.map((plan) => plan.clipId)).size !== clipIds.length) throw new Error(`${definition.chapterId} video-plan clip count mismatch`);
      if (!Array.isArray(packagePlans.clips) || packagePlans.clips.length !== clipIds.length || new Set(packagePlans.clips.map((clip) => clip.id)).size !== clipIds.length) {
        throw new Error(`${definition.chapterId} package clip records do not exactly cover its CSV clips`);
      }
      if (JSON.stringify([...packagePlans.clips.map((clip) => clip.id)].sort()) !== JSON.stringify(clipIds)) throw new Error(`${definition.chapterId} package clip records differ from CSV clip IDs`);
      const hierarchyClipIds = (packagePlans.chapter.sceneIds || []).flatMap((sceneId) => packagePlans.scenes.find((scene) => scene.id === sceneId)?.clipIds || []);
      if (JSON.stringify(hierarchyClipIds) !== JSON.stringify(packagePlans.clips.map((clip) => clip.id))) throw new Error(`${definition.chapterId} package chapter/scene/clip hierarchy differs from package clip order`);
      for (const clip of packagePlans.clips) {
        const plan = packagePlans.videoPlans?.[clip.videoPlanId];
        if (!plan || plan.clipId !== clip.id) throw new Error(`${definition.chapterId} package clip/video-plan binding differs: ${clip.id}`);
      }
      ...
      for (const plan of plans) {
        ...
        if (!clipRows || plan.timelineData?.segments?.length !== clipRows.length) throw new Error(`${definition.chapterId} plan segments missing for ${plan.clipId}`);
        if (promptPackage.clipGlobals.get(plan.clipId) !== plan.globalPrompt) throw new Error(`${definition.chapterId} Markdown/JSON global prompt mismatch: ${plan.clipId}`);
        for (let index = 0; index < clipRows.length; index += 1) {
          ...
          if (timeline.id !== plan.segmentIds[index] || timeline.start !== timing.start || timeline.length !== timing.length || timeline.imageFile !== row.first_frame || timeline.prompt !== prompt.prompt) {
            throw new Error(`${definition.chapterId} plan/CSV/prompt mismatch: ${key}`);
          }
        }
        if (plan.localPrompts !== plan.timelineData.segments.map((segment) => segment.prompt).join(" | ") || plan.segmentLengths !== plan.timelineData.segments.map((segment) => segment.length).join(",")) {
          throw new Error(`${definition.chapterId} flattened plan fields differ from its timeline: ${plan.clipId}`);
        }
      }
    }
    return {
      ...
      consumedHashes: {
        ...
        frameShaJsonSha256: definition.frameShaJson ? sha256File(safeSourcePath(extracted.packageRoot, definition.frameShaJson)) : null,
        frameShaCsvSha256: definition.frameShaCsv ? sha256File(safeSourcePath(extracted.packageRoot, definition.frameShaCsv)) : null,
        fullManifestSha256: definition.shaManifest ? sha256File(safeSourcePath(extracted.packageRoot, definition.shaManifest)) : null,
        continuityLocksSha256: definition.continuityLocks ? sha256File(safeSourcePath(extracted.packageRoot, definition.continuityLocks)) : null
      }
    };
  } catch (error) {
...
export function normalizedGlobalPrompt(packageData, clip, plan) {
  const packageGlobal = packageData.packagePlans
    ? packageData.packagePlans.videoPlans?.[clip.videoPlanId]?.globalPrompt
    : null;
  const runtimePackageGlobal = stripLegacyGlobalDialogue(packageGlobal);
...
function liveQueuePreflight(projectSlug, _clipIds) {
  ...
  for (const url of ["http://127.0.0.1:8188/queue", "http://127.0.0.1:8789/api/queue", "http://127.0.0.1:8791/api/queue"]) {
    ...
    if (Array.isArray(queue.queue_running) && Array.isArray(queue.queue_pending)) {
      ...
      if (running || pending) throw new Error(`Cannot update H06-H09 while live ComfyUI queue is busy (${url}: ${running} running, ${pending} pending)`);
    ...
    if (Array.isArray(queue.jobs)) {
      ...
      if (active.length) throw new Error(`Cannot update H06-H09 while an app queue has any active job (${url}): ${active.map((job) => `${job.id}:${job.status}`).join(", ")}`);
...
function validateStoryboardMapping(storyboard, packages) {
  ...
  if (new Set(allClipIds).size !== allClipIds.length || rowsByKey.size !== 135 || allClipIds.length !== 45) throw new Error("Combined H06-H09 package target set is not exactly 45 clips / 135 segments");
  if (packagePrompts.length !== 135 || new Set(packagePrompts).size !== 135) throw new Error("Combined H06-H09 package does not contain 135 unique segment prompts");
...
function verifyImportedState(storyboard, mediaDirectory, receipt, { packages = [], assets = null } = {}) {
  ...
  if (receipt.packageId !== PACKAGE_ID || receipt.frames?.length !== 135 || receipt.clipIds?.length !== 45) throw new Error("Combined import receipt count or identity is invalid");
  if (new Set(receipt.clipIds).size !== 45 || receipt.frames.reduce((sum, item) => sum + Number(item.sourceReferenceCount || 0), 0) !== 768) {
    throw new Error("Combined import receipt clip/provenance-binding totals are invalid");
  }
  for (const field of ["segmentId", "frameId", "filename"]) {
    if (new Set(receipt.frames.map((item) => item[field])).size !== 135) throw new Error(`Receipt does not contain 135 unique ${field} values`);
  }
  if (new Set(receipt.frames.map((item) => item.prompt)).size !== 135) throw new Error("The 135 imported segment prompts are not unique");
  ...
  if (packages.length) {
    ...
    if (stableHash(receipt.archiveHashes) !== stableHash(expectedArchiveHashes)) throw new Error("Receipt archive hashes differ from the four fixed packages");
  ...
  const h07Relations = {};
  for (const item of receipt.frames.filter((frame) => frame.clipId.startsWith("H07-"))) {
    for (const reference of item.sourceReferences) h07Relations[reference.sourceRelation] = (h07Relations[reference.sourceRelation] || 0) + 1;
  }
  if (JSON.stringify(h07Relations) !== JSON.stringify(PACKAGE_DEFINITIONS.H07.expectedReferenceRelations)) throw new Error("H07 receipt source-relation counts differ from the audited union contract");
  if (receipt.backup) {
...
export function inspectH06H09Packages({ archivePaths = defaultArchivePaths() } = {}) {
  const packages = [];
  try {
    ...
    if (allPrompts.length !== 135 || new Set(allPrompts).size !== 135) throw new Error("Combined packages do not contain 135 unique segment prompts");
    return {
      ok: true,
...
export function importH06H09Ltx25I2vComplete({
  ...
  try {
    ...
    if (active.length) throw new Error(`Cannot update H06-H09 while target generation is active: ${active.join(", ")}`);
    ...
    if (!dryRun) {
      ...
      backup = path.join(backupDirectory, `storyboard.before-h06-h09-i2v-complete.${stamp}.json`);
    ...
    try {
      ...
      for (const packageData of packages) {
        for (const clipId of packageData.clipIds) {
          ...
          if (packageData.promptPackage.negativePrompt) plan.negativePrompt = packageData.promptPackage.negativePrompt;
      ...
      if (stableHash(nonTargetSnapshot(next, targets)) !== beforeNonTargetHash) throw new Error("Import changed storyboard records outside H06-H09");
      if (stableHash(invariantSnapshot(next, targets)) !== beforeInvariantHash) throw new Error("Import changed protected H06-H09 timing, ordering, continuity, history, or semantic-reference state");
      if (dryRun) {
      ...
      if (rollbackErrors.length) throw new AggregateError([error, ...rollbackErrors], "H06-H09 import failed and rollback was not completely clean");
      throw error;
    }
  } finally {
    for (const item of packages) fs.rmSync(item.temporaryRoot, { recursive: true, force: true });
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (modes.length !== 1) {
    console.error("Usage: node scripts/import-h06-h09-ltx25-i2v-complete.mjs (--inspect-packages-only | --verify-only | --apply) [--h06 <exact-zip>] [--h07 <exact-zip>] [--h08 <exact-zip>] [--h09 <exact-zip>]");
    process.exitCode = 2;
  } else try {
    const result = modes[0] === "--inspect-packages-only"
      ? inspectH06H09Packages({ archivePaths })
      : importH06H09Ltx25I2vComplete({ archivePaths, dryRun: modes[0] === "--verify-only" });
